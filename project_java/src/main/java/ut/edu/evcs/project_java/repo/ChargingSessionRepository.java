...
public interface ChargingSessionRepository extends JpaRepository<ChargingSession, String> {
        ...
        @Query("SELECT COUNT(s) FROM ChargingSession s WHERE s.startTime BETWEEN :from AND :to")
        long countSessionsBetween(@Param("from") LocalDateTime from,
                        @Param("to") LocalDateTime to);

        @Query("SELECT COUNT(s) FROM ChargingSession s WHERE s.status = 'COMPLETED' AND s.startTime BETWEEN :from AND :to")
        long countCompletedSessionsBetween(@Param("from") LocalDateTime from,
                        @Param("to") LocalDateTime to);

        @Query("SELECT COUNT(s) FROM ChargingSession s WHERE s.status = 'ACTIVE' AND s.startTime BETWEEN :from AND :to")
        long countActiveSessionsBetween(@Param("from") LocalDateTime from,
                        @Param("to") LocalDateTime to);

        @Query("SELECT COALESCE(SUM(s.kwhDelivered), 0) FROM ChargingSession s WHERE s.status = 'COMPLETED' AND s.startTime BETWEEN :from AND :to")
        ...
        java.util.List<ut.edu.evcs.project_java.domain.session.ChargingSession> findByDriverIdOrderByStartTimeDesc(
                        String driverId);
}
