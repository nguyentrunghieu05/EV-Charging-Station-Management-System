...
public class VNPayController {
    ...
    public VNPayController(IVnPayService vnPayService, InvoiceRepository invoiceRepo,
            ChargingSessionRepository sessionRepo, PaymentRepository paymentRepo) {
    ...
    public ResponseEntity<?> handleReturn(@RequestParam MultiValueMap<String, String> params) {
        try {
                    ...
                    if (invoiceId != null && !invoiceId.isEmpty()) {
                        ...
                        if (inv != null) {
                            ...
                            if (response.isSuccess() && !"PAID".equals(inv.getStatus())) {
                                ...
                                Payment p = new Payment(inv.getId(), "VNPay", "SETTLED", inv.getTotalAmount(),
            ...
            String redirectUrl = "http://localhost:8080/payment.html?status="
            ...
            e.printStackTrace();
            return ResponseEntity.status(302).header("Location", "http://localhost:8080/payment.html?status=error")
                    .build();
        }
    }
