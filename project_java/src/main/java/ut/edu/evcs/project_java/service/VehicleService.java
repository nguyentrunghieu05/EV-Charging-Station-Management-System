...
public class VehicleService {
    ...
    public Vehicle create(Vehicle vehicle) {
        ...
        notificationService.createInAppNotification(
                saved.getDriverId(),
                "Đã thêm xe mới",
                "Xe " + (saved.getBrand() != null ? saved.getBrand() : "") + " "
                        + (saved.getModel() != null ? saved.getModel() : "") + " - biển số " + saved.getPlateNo(),
    ...
    public Vehicle update(String id, Vehicle updates) {
        ...
        if (updates.getBatteryCapacityKWh() > 0)
            existing.setBatteryCapacityKWh(updates.getBatteryCapacityKWh());
        if (updates.getConnectorSupported() != null)
            existing.setConnectorSupported(updates.getConnectorSupported());
        ...
        String metaU = String.format("{\"vehicleId\":\"%s\",\"plateNo\":\"%s\"}", updated.getId(),
                updated.getPlateNo());
        notificationService.createInAppNotification(
                updated.getDriverId(),
                "Cập nhật thông tin xe",
                "Xe " + (updated.getBrand() != null ? updated.getBrand() : "") + " "
                        + (updated.getModel() != null ? updated.getModel() : "") + " đã được cập nhật",
    ...
    public void delete(String id) {
        Vehicle existing = repo.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Vehicle not found: " + id));
        repo.deleteById(id);
        String metaD = String.format("{\"vehicleId\":\"%s\",\"plateNo\":\"%s\"}", existing.getId(),
                existing.getPlateNo());
        notificationService.createInAppNotification(
